...
const pointsProps = [
  { title: 'karty', img: 'Karty', color: '#F5D0C1' },
  { title: 'zetony', img: 'Żetony', color: '#FEFBD7' },
  { title: 'bonusowe', img: 'Karty Bonusowe', color: '#E6E7E9' },
  { title: 'podroz', img: 'Podróż', color: '#C7EAFA' },
  { title: 'wydarzenia', img: 'Wydarzenia', color: '#FEFBD7' }
];

export default class TableEverdell extends Component {
  constructor(props) {
    super(props);
    this.state = {
      users: [],
  ...
  onSubmit = () => {
    fetch('api/games/')
      ...
      .then(data => {
        const { scores } = this.state;
        const gameId = data.find(game => game.name === 'Everdell')._id;
