...
import { Button, Alert, Spinner, Table, Form, Accordion} from 'react-bootstrap';
...
const Result = props => {
  ...
  for(let i=1;i<sortedScores.length;i++)
  {
    ...
    {
      places[i]=places[i-1];
      if(sortedScores[i].points===sortedScores[0].points) winner[i]=sortedScores[i].user.username;
    }
    else places[i]=i+1;
  }
  ...
  return (<tr>
    ...
    <td>
      <Accordion>
        <Accordion.Toggle as={Button} size="sm" block variant="primary" eventKey="0">
          All scores
        </Accordion.Toggle>
